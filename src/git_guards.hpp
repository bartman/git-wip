...
struct RepoGuard {
    ...
    ~RepoGuard() { if (m_repo) git_repository_free(m_repo); }
...
struct IndexGuard {
    ...
    ~IndexGuard() { if (m_idx) git_index_free(m_idx); }
...
struct TreeGuard {
    ...
    ~TreeGuard() { if (m_tree) git_tree_free(m_tree); }
...
struct CommitGuard {
    ...
    ~CommitGuard() { if (m_commit) git_commit_free(m_commit); }
...
struct ReferenceGuard {
    ...
    ~ReferenceGuard() { if (m_ref) git_reference_free(m_ref); }
...
struct SignatureGuard {
    ...
    ~SignatureGuard() { if (m_sig) git_signature_free(m_sig); }
...
struct RevwalkGuard {
    ...
    ~RevwalkGuard() { if (m_walk) git_revwalk_free(m_walk); }
